...
#define ledRetroiluminacion 0
#define fotoResistencia A5
...
int puntoEncendidoRetroiluminacion = 500; //Modificar valor para ajustar la retroiluminacion automática
...
void setup() {
  ...
  pinMode(ledRetroiluminacion,OUTPUT);
}
...
void loop() {
  ...
  int lecturaFotoResistencia = analogRead(fotoResistencia);
  

  controlarTemperatura(temperaturaActual,giroDer);
  modoNumerosPrimos = verificarModo(estadoInterruptor);
  controlarRetroiluminacion(lecturaFotoResistencia,ledRetroiluminacion, puntoEncendidoRetroiluminacion);
...
int controlarBotonReiniciar(int estadoBotonActual,int ultimoEstadoBoton, int contador)
{
  ...
  void controlarRetroiluminacion(int valorFotoResistencia,int ledDeRetroiluminacion, int puntoDeEncendido)
  {
      if (valorFotoResistencia <= puntoDeEncendido)
      {
        digitalWrite(ledDeRetroiluminacion,HIGH);
      }
        else
        {
          digitalWrite(ledRetroiluminacion,LOW);
        }
  }
